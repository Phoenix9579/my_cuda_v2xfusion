...
def get_files_path(path_my_dir, extention=".json"):
    ...
    for (dirpath, dirnames, filenames) in os.walk(path_my_dir):
        for filename in filenames:#filenames: 当前文件夹下的文件名列表
            if os.path.splitext(filename)[1] == extention:#使用 os.path.splitext(...) 分割文件名和扩展名;只保留扩展名与参数 extention 相同的文件（如 .json）
                path_list.append(os.path.join(dirpath, filename))#把符合条件的文件的完整路径加入到 path_list 列表中
    return path_list
